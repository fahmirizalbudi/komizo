...
import { createComic, updateComic } from '@/app/admin/comics/actions'
import { ComicEditInput, ComicWithGenresAndChapters } from '@/types/comic'

type ComicEditFormProps = {
  initialComic: ComicWithGenresAndChapters
}

const ComicEditForm = ({ initialComic }: ComicEditFormProps) => {
  const router = useRouter()
  const [genreOptions, setGenreOptions] = useState<SelectOption[]>([])
  const [comic, setComic] = useState<ComicEditInput>({
    id: initialComic.id,
  ...
  useEffect(() => {
    if (initialComic.genres?.length) {
      setSelectedGenreIds(initialComic.genres.map((g) => g.genre.id))
    }

    if (initialComic.coverUrl) {
      setCoverPreview(initialComic.coverUrl)
    }
  }, [initialComic])
  ...
  const handleSave = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    await updateComic(payload, cover ?? undefined)
